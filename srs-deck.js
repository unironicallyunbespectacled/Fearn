...
(function (global) {
  ...
  function openSRSFlashcardDeck() {
    var existing = document.getElementById('fearn-srs-deck-modal');
    ...
    titleRow.innerHTML = '<div style="font-size:1.1rem; font-weight:800; color:#38bdf8; display:flex; align-items:center; gap:8px;">🎴 <span>Spaced Repetition Review Deck</span></div><button id="fearn-srs-close" style="background:transparent; border:none; color:#94a3b8; font-size:1.4rem; cursor:pointer;">✕</button>';
    ...
    document.getElementById('fearn-srs-close').onclick = function () {
    ...
    rateBtns.forEach(function (btn) {
      btn.onclick = function () {
        if (overlay.parentNode) overlay.parentNode.removeChild(overlay);
        if (window.FEARN && window.FEARN.ui && typeof window.FEARN.ui.celebrateCheckpoint === 'function') {
          window.FEARN.ui.celebrateCheckpoint({ message: '🧠 SRS Memory Anchor Logged!' });
        }
      };
    });
