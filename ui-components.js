...
(function (global) {
  ...
  function formatMathAndText(str) {
    ...
    // Mathematical symbols & relations
    const symbols = {
      '\\\\le(?!t)': '≤', '\\\\leq': '≤', '\\\\ge': '≥', '\\\\geq': '≥',
      '\\\\ne(?!w)': '≠', '\\\\neq': '≠', '\\\\approx': '≈', '\\\\equiv': '≡',
      '\\\\sim': '∼', '\\\\propto': '∝', '\\\\pm': '±', '\\\\mp': '∓',
    ...
    // Braced super and subscripts
    s = s.replace(/\^\{([^}]+)\}/g, '<sup>$1</sup>');
    s = s.replace(/_\{([^}]+)\}/g, '<sub>$1</sub>');

    // Common non-braced superscripts
    s = s.replace(/\^2(?![0-9])/g, '²')
         .replace(/\^3(?![0-9])/g, '³')
         .replace(/\^0(?![0-9])/g, '⁰')
         .replace(/\^1(?![0-9])/g, '¹')
         .replace(/\^n(?![a-zA-Z])/g, 'ⁿ')
         .replace(/\^t(?![a-zA-Z])/g, 'ᵗ')
         .replace(/\^k(?![a-zA-Z])/g, 'ᵏ')
         .replace(/\^([0-9]+)/g, '<sup>$1</sup>');

    // Common non-braced subscripts
    s = s.replace(/_([0-9a-zA-Z]+)/g, '<sub>$1</sub>');

    // Display math: $$ ... $$ -> formatted block
    ...
    // Inline math: $ ... $ -> styled inline math span
