// FEARN University Elite Module — CS110: Algorithms & Data Structures
// Registers 'skill-cs110'. Renders the syllabus from cs110 curriculum,
// interactive mathematical proof inspector, asymptotic runtime analyzer,
// algorithm visualizers, lesson presentations, and rigorous checkpoint examinations.
...
(function (global) {
  ...
  function defaultLessonState() {
    ...
    return { currentLessonId: ledger.authoredInFull[0] || 'cs110-u1-l1', completedLessons: [] };
  }
  ...
  function render(container) {
    ...
    root.className = 'fearn-uni-module cs110-container';
    ...
    header.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px;">
        <div>
          <span style="font-size:0.8rem; text-transform:uppercase; letter-spacing:0.08em; color:var(--accent-color, #38bdf8); font-weight:700;">University Elite Course</span>
          <h1 style="margin:4px 0 0; font-size:1.8rem; font-weight:800; display:flex; align-items:center; gap:10px;">
            <span>💻</span> CS110: Algorithms & Data Structures
          </h1>
          <p style="margin:6px 0 0; color:var(--text-muted, #94a3b8); font-size:0.95rem;">
            Rigorous mathematical foundations, asymptotic analysis, balanced search trees, dynamic programming, and greedy graph theory.
          </p>
        </div>
        <div style="background:rgba(56,189,248,0.1); border:1px solid rgba(56,189,248,0.3); border-radius:10px; padding:10px 18px; text-align:right;">
    ...
    // Main 2-column layout: Left = Syllabus Session Picker, Right = Interactive Lecture & Problem Set
    const mainGrid = document.createElement('div');
    mainGrid.style.display = 'grid';
    mainGrid.style.gridTemplateColumns = 'minmax(280px, 320px) 1fr';
    mainGrid.style.gap = '24px';

    // Left Column: 25-Session Academic Syllabus
    const leftCol = document.createElement('div');
    leftCol.className = 'cs110-syllabus-pane';
    ...
    (curr.units || curr.roadmap || []).forEach((unit, uIdx) => {
      ...
      (unit.lessonIds || unit.lessons || []).forEach((lId, sIdx) => {
        ...
        btn.style.border = isSelected ? '1px solid #38bdf8' : '1px solid transparent';
        btn.style.background = isSelected ? 'rgba(56,189,248,0.2)' : (isDone ? 'rgba(34,197,94,0.1)' : 'transparent');
        ...
        const title = isAuthored ? (curr.lessons[lId].objective.split('(')[0].trim()) : `Session ${sIdx + 1}: Planned Lecture`;
        btn.innerHTML = `
    ...
    rightCol.className = 'cs110-lecture-pane';
      ...
      rightCol.innerHTML = `
        <div style="border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:16px; margin-bottom:20px;">
          <div style="display:flex; justify-content:space-between; align-items:center;">
            <span style="font-size:0.8rem; font-weight:700; color:#38bdf8; background:rgba(56,189,248,0.1); padding:4px 10px; border-radius:4px;">
        ...
        <!-- Academic Presentation & Mathematical Exposition -->
...
${formatRich(activeLesson.presentation.explanation)}
        ...
        ${activeLesson.presentation.examples && activeLesson.presentation.examples.length ? `
          <div style="margin-bottom:24px;">
            <h4 style="margin:0 0 10px; font-size:1rem; color:#a78bfa; text-transform:uppercase; letter-spacing:0.05em;">
              ⚡ Algorithmic Examples & Invariant Analysis
            </h4>
        ...
        <div id="cs110-exam-container" style="background:rgba(15, 23, 42, 0.9); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:20px; margin-top:24px;">
          ...
          <div id="cs110-quiz-area"></div>
        </div>
      `;

      // Mount Quiz synchronously
      const quizArea = rightCol.querySelector ? rightCol.querySelector('#cs110-quiz-area') : null;
  ...
  FEARN.registerModule({
    id: MODULE_ID,
    name: 'CS110: Algorithms & Data Structures',
    icon: '💻',
    category: 'skill',
    render: render,
