...
3. Fix Section 0b, Section 0d, and Section 0f together (Ukrainian, Hindi, Urdu, Swahili templated `explanation` text; Japanese, Russian, Amharic, Arabic, Cantonese, Mandarin, and Korean's boilerplate `mnemonics`/`culturalNotes`; the leaked-ID checkpoint bug confirmed in Hindi, Korean, Swahili, Ukrainian, and Urdu but not yet ruled out anywhere else) — full re-authoring or cleanup of every affected field, in every affected language, to the same real-content bar as French's and Spanish's current state.
...
## 3. Named anti-patterns — the complete catalogue, 29 entries, all previously confirmed on this exact repo or its adjacent proposals
