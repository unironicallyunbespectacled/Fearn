...
- **Fitness has its own THIRD, separate speech implementation — do not confuse this with `FEARN.audio`.** `modules/fitness.js` (around line 316-328) has its own "voice coach" feature, gated behind a `getVoiceCoachEnabled()` toggle, with its own independent `window.speechSynthesis.speak()` call. This is a genuinely separate audio system from `FEARN.audio.speak` (language lessons) and `FEARN_PLAY_MORSE` (Morse tones) — three separate implementations exist in this codebase, not two. **Fitness does not need new audio added — it needs its existing voice-coach implementation checked for the same silent-audio root cause as Problem 1** (does it have the same synchronous-user-gesture issue, a similar `cancel()`/`speak()` race, etc.?), and verified live that it actually produces sound when the toggle is on. Correct any earlier assumption that Fitness has "zero audio" — it doesn't; it has its own system that may share the same underlying bug.
- The 3 university-course Fronds (b110, b111, cs110) — these ARE real, separate files at `modules/skills/b110.js`, `modules/skills/b111.js`, `modules/skills/cs110.js` (confirmed by direct inspection — they live under `modules/skills/`, not a separate `modules/courses/` directory). Confirmed zero audio integration in any of the three. Determine if spoken audio applies to their content in any meaningful way, and either add it or report explicitly why it doesn't apply.
...
For each of Chess, Scrabble, Typing, Songwriting, and Mental Math: determine whether spoken audio genuinely makes sense for that Frond's content (e.g., Scrabble might benefit from hearing a word spoken; Chess might benefit from a move being read aloud in algebraic notation; Typing/Mental Math may have little to no legitimate use for speech synthesis, being non-linguistic). Where it genuinely makes sense, add it using the same `FEARN.audio` infrastructure. Where it doesn't, explicitly report why not, rather than silently skipping the Frond. Do not force speech synthesis onto content where it wouldn't actually help a learner, just to claim "audio coverage" — that would be exactly the kind of hollow compliance this project's standards forbid.

**For Fitness specifically**, do not add anything new — instead debug its existing voice-coach implementation (see above) for the same root cause as Problem 1, and verify live that it actually speaks when enabled.

### Part 4: The 3 course Fronds (b110, b111, cs110)
Located at `modules/skills/b110.js`, `modules/skills/b111.js`, `modules/skills/cs110.js`. Determine if spoken audio applies to their content in any meaningful way, and either add it or report explicitly why it doesn't apply.
