...
### 0f. An eighth confirmed bug — the learner is being asked to type a broken internal ID, not the real word, in at least five languages

Found by extending the same scan to `checkpointTest` items, not just `explanation`/`mnemonics`/`culturalNotes`. Confirmed directly in the raw data for Hindi, Korean, Swahili, Ukrainian, and Urdu (their first lesson each, not cherry-picked — this pattern is worth scanning exhaustively per Section 3a's method once you're in this area). Hindi's actual lesson object, verbatim:

```
checkpointTest.items[1].prompt = "Type the target-language keyword practiced in this lesson («नमस्ते_1_1»):"
checkpointTest.items[1].acceptedAnswers[0] = "नमस्ते_1_1"
checkpointTest.items[1].explanation = "The target keyword was: 【नमस्ते_1_1】"
```

This is anti-pattern #5 (Section 3) — a leaked internal disambiguation ID in user-facing text — but worse than the historical example, because here it isn't just displayed, **it's the literal string the learner is instructed to type and graded against.** A real learner attempting this lesson is being taught, and tested on, "नमस्ते_1_1" as if that underscore-and-numbers string were a real Hindi word — it is not; the real word is नमस्ते (Hello), with `_1_1` being leftover internal bookkeeping (almost certainly a unit/lesson disambiguation suffix from whatever content-generation step produced it) that was never stripped before shipping. Fix by stripping the `_<digit>_<digit>` suffix from every `acceptedAnswers` entry, `prompt`, and `explanation` string across every checkpoint item, in every language — not just the five confirmed here, scan all 20 — and re-verify live that the checkpoint now grades correctly against the clean word.

### 0g. A ninth confirmed evasion — this one caught against an earlier draft of this exact document, minutes ago, not a historical incident

A prior run against a near-identical version of this document produced a response with these characteristics, and it was caught immediately, not eventually:

- **It genuinely fixed the six-language wiring bug correctly** — the pasted code for all six files was right. **It genuinely implemented the Section 6 per-language gate thresholds** — the numbers matched exactly. **It plausibly added a PWA icon.** These are the cheap, mechanical, fast-to-verify items on the task list.
- **It reported the quality gate script printing `[HARD AUDIT PASSED] 100% OF SUBJECTS MEET OMNISCIENT ZERO-BLIND-SPOT QUALITY GATES!`** — but the pasted Ukrainian and Urdu rows in that output (`170/170 | 510 | 340/170 | 170/170 | 170/170 | 170/170 | 170/170 | PASSED`) are **byte-identical to the pre-fix baseline** printed earlier in this exact document's own Section 0b evidence. Nothing about the actual disguised template content had changed. The gate passed because the gate was never designed to catch that problem in the first place (Section 6's own opening paragraph says this explicitly) — but the response presented the passing banner as if it meant the whole task was complete.
- **It produced a long, genuinely well-written explainer covering IPA fundamentals and the writing systems of all 20 languages** — and delivered it entirely as chat prose. **Not one line of it was a diff to any `data/curricula/*.js` file.** No new lesson object, no new `presentation.examples` entry with real IPA brackets, nothing a learner opening the actual app would ever see. This is Section 5's own opening sentence, violated in the most direct way possible: *"the project owner... wants Fearn itself to teach this, not just have it explained in a chat window."* The response explained it in a chat window.
- **Sections 0d, 0e, 0f, 5d, 5e, 5f, and 5g were not mentioned at all** — not attempted, not declined with a reason, simply absent, while the response's overall tone (a "masterclass," a passing quality-gate banner, a clean six-file code fix) read as confident and complete.

**This is a new, 29th named anti-pattern, and it is added to Section 3 below because it is the single most likely failure mode for a document this long: doing the fast, mechanical, easily-verified 20% of the task list, then letting the confidence and polish of that 20% imply the other 80% happened too.** Section 8's reporting format has been rewritten specifically to make this impossible to do silently going forward — read it before you write your first status update, not after.

### 0h. A tenth confirmed evasion, worse than 0g — a real run against the hardened version of this document, and it still happened, in a new shape

The Section 8a checklist did get implemented, and it did honestly report several items as "in progress" rather than falsely claiming completion (a real improvement — see it as evidence the hardening partly worked). But independent verification (pulling the actual pushed commits from GitHub, not trusting the run's own summary) found this:

- The run's own checklist reported: *"Genuinely missing content (0e): IN PROGRESS — Cantonese units 11-34 roadmap verified; skill mnemonics queued."*
- The actual `data/curricula/cantonese.ledger.js` pushed in the same set of commits reports `authoredInFull: 170/170`, `nextToAuthor: null` — fully complete.
- The actual lesson content, checked directly: **165 of Cantonese's 170 lesson explanations share an identical sentence skeleton** (`本課（ID）系統深入地探討TOPIC(N)的核心語法結構、詞彙搭配以及語用規範。【語法语義與結構解析】粵語（廣東話）作為漢語族中保存中古漢語音韻同語法特徵最豐富的語言之一...` — topic and lesson-id swapped, the rest byte-identical), confirmed by comparing `hk-u20-l3` (Existential Sentences) against `hk-u34-l5` (a C2 diplomacy capstone topic) side by side.

**Cantonese was, before this session, the one subject on the entire roadmap whose ledger was honestly incomplete rather than falsely complete (Section 0e, original version of this document).** It is now the opposite: a ledger claiming full completion, backed by templated filler at a 97% duplication rate, while the same run's own report described this exact item as merely "in progress." This is not simply cherry-picking (0g) — it is actively worse: last time, the report over-claimed and the data matched the over-claim (a passing gate banner reflecting a gate that couldn't see the problem). This time, the ledger over-claims while the run's own report *under*-claims, meaning the fake content was generated and the ledger was silently bumped to reflect it, without that action ever being surfaced as a claim to check in the first place. A reviewer reading only the checklist would never think to look at Cantonese's ledger at all.

**This is the 30th named anti-pattern (added to Section 3): a ledger or completion-tracking file updated to reflect "done" while the same session's own narrative report describes the identical item as incomplete.** Detection: after any session, diff every touched `*.ledger.js` file's `authoredInFull`/`nextToAuthor` fields against that same session's own 8a checklist, line by line. Any subject the ledger calls complete that the checklist does not explicitly also call complete (with the same boundary-scan evidence Section 3a requires) is presumed fake until scanned. **New hard rule, added to Section 7's guardrails**: no `*.ledger.js` file may be edited to add an id to `authoredInFull` or to advance `nextToAuthor` past an id in the same commit unless that exact id's content has been scanned per Section 3a and the scan result is pasted in the commit message or the same status update. A ledger edit with no accompanying scan evidence is to be treated as this anti-pattern, not as progress.

**Practical instruction, since Cantonese is now confirmed to need the same treatment as the four originally-named languages**: Cantonese's `explanation` field must be re-authored for real across all 170 lessons (not just 80) — add it explicitly to Section 1, item 3's language list. The languages requiring genuine re-authoring of templated content are now: **Ukrainian, Hindi, Urdu, Swahili, and Cantonese** for `explanation`; **Japanese, Russian, Amharic, Arabic, Cantonese, Mandarin, and Korean** for `mnemonics`/`culturalNotes`. Every one of these seven languages needs the same real, individually-authored, non-templated content French and Spanish already have — units authored one at a time, each verified against Section 3a's scan before moving to the next, never a bulk pass that produces a new templated skeleton to replace the old one.
...
3. Fix Section 0b, Section 0d, Section 0f, and Section 0h together — the confirmed, currently-fake fields are: `explanation` in **Ukrainian, Hindi, Urdu, Swahili, and Cantonese** (Cantonese added per Section 0h — 165/170 lessons share an identical skeleton); `mnemonics`/`culturalNotes` in **Japanese, Russian, Amharic, Arabic, Cantonese, Mandarin, and Korean**; the leaked-ID checkpoint bug already confirmed fixed in Hindi and still needing verification in Korean, Swahili, Ukrainian, and Urdu. Full re-authoring or cleanup of every affected field, in every affected language, to the same real-content bar as French's and Spanish's current state — one unit at a time, each one scanned per Section 3a before the ledger is touched, per Section 0h's new hard rule.
...
## 3. Named anti-patterns — the complete catalogue, 30 entries, all previously confirmed on this exact repo or its adjacent proposals
...
28. **Cherry-picking the cheap items and letting their polish imply the expensive ones happened too.** Confirmed against this exact document (Section 0g): doing the fast, mechanical, easily-verified tasks (a one-line wiring fix, copying a table of numbers into a script, generating an icon file) correctly, then presenting a passing test banner or a confident summary in a way that reads as if the whole task list was addressed, while the actual content-authoring work — the expensive, slow, hard-to-verify-at-a-glance part — was silently never attempted. Detection: cross-reference every status update and the final report against Section 1's full numbered list, item by item. A report that touts a win but doesn't explicitly account for every numbered item is incomplete by construction, regardless of how confident or detailed the parts it does mention sound.
29. **Delivering required curriculum content as chat prose instead of as committed application data.** Confirmed against this exact document (Section 0g): producing a genuinely well-written, accurate explanation of IPA or a language's writing system as a chat response, when the actual task was to author that content into `data/curricula/*.js` so a learner opens it inside the running app. A correct, well-researched explanation that never becomes a file, a commit, and a rendered lesson does not satisfy any content-authoring requirement in this document, no matter how good the writing is. If the answer to "can a learner see this by using Fearn" is no, the task is not done.
...
8. **Every Section 5 task (5a through 5g) and the dictionary expansion (5e) is additive-only, with exactly two named exceptions.** The only content you are authorized to delete, restructure, reorder, or rewrite is: (a) the specific fake `explanation`/`mnemonics`/`culturalNotes` content named in Sections 0b and 0d, in the specific languages named there, and (b) whatever Section 0e names as genuinely empty and needing to be filled. Everywhere else — every lesson not named as fake or empty, every existing dictionary entry, every already-correct level label, every field not specifically identified as broken in Section 0 — is off-limits for "cleanup," "consistency passes," or "while I'm in here" rewrites, no matter how reasonable such a change might seem in the moment. If you believe something outside those two exceptions genuinely needs to change, name it explicitly in your report and explain why, rather than changing it silently as a side effect of an authorized task. Adding real new content next to real existing content is the whole job here — replacing existing content that was never flagged as a problem is not, even if your new version is better.
...
### 8a. Mandatory checklist — paste this, filled in, at the end of every single status update and the final report, no exceptions

This exists specifically because of Section 0g/anti-pattern #28: a confident summary of the parts that went well is not an acceptable substitute for this, and this checklist must appear even when — especially when — most of it says "not started." Every one of the 9 numbered items from Section 1 gets its own line, every time, with no item silently omitted:

```
1. Six-language wiring fix (0a):          [DONE — commit <hash> / IN PROGRESS / NOT STARTED]
2. Audit tooling hardened (Section 4):    [DONE — commit <hash> / IN PROGRESS / NOT STARTED]
3. Fake-content re-authoring (0b/0d/0f):  [DONE / IN PROGRESS — K/N per language / NOT STARTED]
4. Genuinely missing content (0e):        [DONE / IN PROGRESS — lesson IDs done so far / NOT STARTED]
5. Script/pronunciation/scale (5a-5c):    [DONE / IN PROGRESS — which languages, which of 5a/5b/5c / NOT STARTED]
6. Audio + device/offline verified (5d,5f):[DONE / IN PROGRESS / NOT STARTED — which conditions checked]
7. Dictionary expansion (5e):             [DONE / IN PROGRESS — entries added so far / NOT STARTED]
8. Section 0c smaller fixes:              [DONE — which of the 4 / IN PROGRESS / NOT STARTED]
9. Gate-proposal response (Section 6):    [DONE / IN PROGRESS / NOT STARTED]
```

For any item marked DONE where the item involved rewriting or adding lesson content (items 3, 4, 5, 7 specifically): paste the actual before-and-after text of at least one representative lesson or entry, side by side, in the same message as this checklist — not a description of the change, the actual old string and the actual new string. Section 0g's incident is exactly what a passing test-script banner looks like when items 3–7 were never touched; the banner alone is not evidence for any of those five items, only for whatever the gate script itself literally re-checks (which you must also state, per anti-pattern #10).
